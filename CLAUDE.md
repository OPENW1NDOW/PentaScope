...
运行需在项目根目录配置 `.env`（`DOUBAO_API_KEY` 必填，其余见 `src/utils/config.py` 默认值）。
...
- **反馈闭环** `builder.py::should_continue`：质检不通过时，按 `feedback.issues[].agent` 字段决定打回 `collector` 还是 `writer`；`retry_count >= max_retries`（默认 2）则强制结束。这是课题要求的「质检→采集反馈闭环」的实现点。
- **Agent**（`src/agents/`）：均为持有 `llm`/`http`/`parser` 依赖的类，对外暴露单个 async 方法（`collect`/`analyze`/`write`/`inspect`）。`collector` 内部三步走：目标解析 → 竞品分类 → 差异化采集（见 DECISIONS.md）。所有 prompt 集中在 `src/agents/prompts.py`。
