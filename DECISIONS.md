...
- 理由：原 plan 假设 D+F 与 E+G 跨 worktree 文件无冲突可并行，但只看了"写文件层面"未看"接口契约层面"。实际三条强依赖：
  - E↔F：inspector 硬查的字段路径必须和 writer 实际产出对齐，否则直接挂
  - E↔D：builder.writer_node 调用 WriterOrchestrator 的 7 参数签名必须由 E 敲定
  - E↔G：前端按 BaseReport.scenario_payload 字段渲染，writer 字段名变了前端就崩
  - worktree A 完成后留下大量隐含接口假设（writer.py NotImplementedError 桩、builder.writer_node 的 pass、AnalysisState 字段路径、inspector 已写好的硬查路径）。这些假设没写在 spec 里、只藏在代码细节中。worktree B 接手会处处踩到 A 当时的隐含决定，对齐成本可能超过重做代价
- 备选 A：worktree B rebase 后接着做（排除：见上述对齐成本风险）
- 备选 B：作废全部 worktree 进度，从 b412b0b 起步重做（排除：normalizers 5 套 + 25 测试已扎实落地、与 worktree A 假设无关，丢了浪费 ~500-800 行代码 + 半轮 doubt-driven）
- 备选 C：保留双 worktree 现状继续并行（排除：Cooper 06-08 判断"实则相互依赖"，并行假设破灭）
- 影响：
  - master git log 会保留 D+F 做了又撤的痕迹（事实记录，不强推）
  - worktree A 留下的 5 段 06-08 决策（writer_node pass / D Step 0 / quality_score 模块化 / F 5 套测试范围 / worktree 模式迁移）历史保留作记录但已作废，新决策走单 session 重做的逻辑
  - 后续 D 重做时 prior_trace_id 读盘归属、builder 错误处理路径等都重新设计，不继承 worktree A 的假设
  - 远程 worktree-scenario-schemas-graph / worktree-scenario-writer-frontend 分支保留作历史

---

## 2026-06-08: writer_node D 阶段暂 pass，等 E 大类 WriterOrchestrator 接入【已作废，见 06-08 晚】
- 选择：本 worktree 改 builder.py 时，writer_node 改为 `return {"current_node": "writer"}` 不调任何 writer，state.report 留空；inspector_node 加 None 兜底（report is None → log warning + skip 质检）
- 理由：旧 WriterAgent 产 FinalReport，已被 A 大类废除；新 WriterOrchestrator 是 E 大类（worktree-scenario-writer-frontend）任务。D session 没必要先做适配性的桩 writer，桩了下次还要拆——直接让 graph 在 D 阶段「跑得起来但不产报告」，E 接入时整文件替换 writer_node
- 备选 A：保留旧 WriterAgent 适配 FinalReport→BaseReport（排除：旧 writer 100+ 行依赖了 BaseReport 没有的字段如 action_items/swot/radar_scores，适配工作量等于重写一半，浪费）
- 备选 C：返回最小 BaseReport stub 让 graph 跑出占位报告（排除：测试时不知道 stub 在哪触发幻觉，调试成本高于价值）
- 影响：D 阶段不能跑 graph 端到端测试，本 worktree 只能跑 inspector 单测；E 合回 master 后才能跑端到端

## 2026-06-08: D Step 0 把 A+B+C 留下的 FinalReport 引用一次清干净【已作废，见 06-08 晚】
- 选择：本 worktree 进入 D 大类前，先把 A+B+C session 留下的 3 处 FinalReport 引用（state.py / writer.py / inspector.py）一次修齐，作为 Task 15 的前置 Step 0；不让 E session 再处理一次
- 理由：A+B+C session 只删了 schemas 层 FinalReport（按 06-07（晚）DECISIONS 的「治本清理 + 过渡 skip」策略），但 graph + agents 层这 3 个文件还在 import 它，运行时全崩。E session 也会改 writer.py，如果让 E 处理，D session 写代码时无法 `python -c "from src.graph.builder import ..."` 验证。D 一次修齐让两个 worktree 的工作不互相阻塞
- 备选 A：让 E session 处理（排除：D 写不了代码，也无法跑 inspector 单测）
- 备选 B：A+B+C session 重新启动一次补这道（排除：折返成本高，session 数量上升）
- 影响：D 单 commit 跨「Step 0 适配 + Task 15-18 + Task 23」，commit 范围比 plan 设计的 D+F 更宽

## 2026-06-08: inspector quality_score 公式集中到独立模块 src/agents/quality_score.py【已作废，见 06-08 晚】
- 选择：spec Part 4.5 quality_score 三项加权公式（source_coverage / confidence_avg / inspector_pass_rate）实现成独立模块 `src/agents/quality_score.py`，inspector.py 只调用 `calc_quality_score(report, issues)` 一行；同时把 builder.py 内原 penalty 公式 `1.0 - sum(penalty[severity])` 删掉（去重）
- 理由：① 公式独立可测：`tests/unit/test_inspector_scenario.py` 直接 import calc_source_coverage / calc_confidence_avg / calc_inspector_pass_rate / calc_quality_score 4 个函数做单测，不用绕一层 inspector.inspect ② 公式定义有数学层语义（递归遍历 BaseModel + 缺值降权重新归一化），与 inspector 的"按 scenario 分支硬查"语义不同，分文件减小 inspector.py 体积
- 配套规则：
  - source_coverage 颗粒度细（Cooper 决策 Q1=a）：`feature_matrix.categories[].features[]` 每个 entry 各贡献 1 个分母
  - inspector_pass_rate 公式（Cooper 决策 Q2=c）：复用 builder.py 原 `1.0 - sum(penalty)` clamp[0,1]（critical=0.4 / major=0.2 / minor=0.05）
  - confidence_avg 归一化映射：high/from_user_brief=1.0 / medium/llm_inferred=0.6 / low/low_confidence=0.3（兼容 PositioningStatement 的 3 种特殊枚举）
  - 缺值降权：source_coverage / confidence_avg 哪一项 None 就重新归一化剩余权重（避免空 confidence 把整个分数拉到 0）
- 备选 A：公式留在 inspector.py 内（排除：测试公式还得构造完整 inspector 实例，单测脏乱）
- 备选 B：复用 builder.py 现有 penalty 公式不升级（排除：spec Part 4.5 是 doubt-driven 已通过的设计，跳过等于答辩故事缩水）
- 备选 C：spec 说「严重度加权和 / 满分」按 issue 总数动态算满分（排除：单 critical 和 10 个 critical 落同区间，不合理）

## 2026-06-08: F Task 23 5 套硬查规则覆盖范围分两层——实现 5 套，测试主测 S1+S2【已作废，见 06-08 晚】
- 选择：inspector.py 把 _check_s1..s5 五套硬查规则全部实现（vendor_profiles 覆盖 / market_sizing 全 unknown / packaging 强制 1 个 is_recommended / 5 类 changes 全空 / perceptual_map.display_watermark 等共 ~15 条规则）；测试层只用现有 schema fixture 测 S1+S2 关键路径 + S3/S4/S5 通过 dispatcher 路由测试覆盖（验 _check_sX 被路由到，不验具体规则）
- 理由：① S3/S4/S5 没有 `_make_minimal_s3/s4/s5_payload` fixture，从零造每套要 ~80 行 fixture 代码（5 套合计 ~400 行测试支撑代码），ROI 低 ② 具体规则的语义对错由 H 大类端到端测试覆盖（5 场景真实跑通时若规则误报会暴露） ③ Cooper 拍板 Q2=A 完整实现 5 套规则，测试范围用 b 方案（不重写 fixture）
- 备选 A：硬刚 5 套完整 fixture（排除：~500 行测试代码 ROI 低，H 大类端到端会覆盖）
- 备选 C：5 套规则全留 placeholder（排除：D 阶段就该把规则写完，E session 才能依赖 inspector 反馈来调 writer）
- 影响：H 大类端到端验收时如果 S3/S4/S5 某条规则误报或漏报，本 worktree 没专项测试可定位，需在 H 大类时回头补单测

## 2026-06-08: worktree 模式从项目自管 .worktrees/ 迁到 Claude Code 原生模式【保留，与 D+F 作废无关】
- 选择：worktree 路径从项目自管 `.worktrees/`（已被 .gitignore 单独 ignore）迁到 Claude Code 原生 `.claude/worktrees/<name>/`（已被 `.claude/` 规则覆盖，无需新加 ignore）；新增 `.worktreeinclude` 让新 worktree 自动复制 `.env`/`.env.local`，不用手动 cp；启动方式改为 `claude --worktree <name>`，不再 `git worktree add`
- 理由：① Claude Code 原生 worktree 自动管理路径 + 分支 + 退出策略（keep/remove） ② `.worktreeinclude` 解决了之前手动 cp .env 的痛点 ③ 路径统一在 `.claude/worktrees/` 利于多 session 并行管理 ④ 退出 session 时若改动未提交，自动提示 keep/remove 防误删
- 备选 A：保留 .worktrees/ 手动管理（排除：路径不统一、需要手动 cp .env、无 keep/remove 提示）
- 影响：原 .worktrees/ 空目录已删；PROGRESS 06-07 启动指引同步更新；本次 D+F 即在 .claude/worktrees/scenario-schemas-graph/ 中完成
