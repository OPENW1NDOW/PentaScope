...
> 启动前需在项目根目录配置 `.env`，至少包含 `DOUBAO_API_KEY`；S2 场景额外需要 `TAVILY_API_KEY`。详情见 README。
...
| 输入文案 | 预期推荐 | 关键词依据 |
|---|---|---|
| `我们已经有协作文档产品，想看竞品有哪些功能我们没做，决定下个迭代要补什么` | S1 | 已有产品 + 看竞品功能差距 |
| `我准备进入 AI 编程工具赛道，目前还没产品，想调研一下市场头部玩家有哪些` | S2 | 还没产品 + 市场调研 + 头部玩家 |
| `我们的 SaaS 产品想调价格，用户反馈现在太贵，想看竞品怎么定价` | S3 | 调价 + 竞品定价 |
...
> 你也可以把 `Notion` 替换为任何已有产品，把竞品换成同赛道 2-5 个对手。
...
> 行业关键词可以替换为任意细分赛道，例如「企业 IM」「数据库 SaaS」「跨境电商 ERP」。
...
| 后端报 LLM 401 | 检查 `.env` 里 `DOUBAO_API_KEY` 是否正确 |
