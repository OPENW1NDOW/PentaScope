...
本文档为 v3 spec 实施完成后的手动验收脚本。每个场景包含：
- **测试输入**（直接复制粘贴到前端表单）
- **重点关注**（验收时盯哪些点）
- **预期产出**（成功标准）
...
**目的**：验证 `/api/v1/pick-scenario` 端点 + 前端按钮 + 推荐回填到 selectbox
...
| 输入文案 | 预期推荐 | 为什么 |
|---|---|---|
| `我们已经有协作文档产品，想看竞品有哪些功能我们没做，决定下个迭代要补什么` | S1 | 关键词：已有产品 + 看竞品功能差距 |
| `我准备进入 AI 编程工具赛道，目前还没产品，想调研一下市场头部玩家有哪些` | S2 | 关键词：还没产品 + 市场调研 + 头部玩家 |
| `我们的 SaaS 产品想调价格，用户反馈现在太贵，想看竞品怎么定价` | S3 | 关键词：调价 + 竞品定价 |
| `我们每个季度都跟踪几个核心竞品的最新动作，这次想更新一下战卡` | S4 | 关键词：每季度跟踪 + 战卡（监控） |
| `我们的产品市场定位有点尴尬，想重新做一次定位、找到差异化` | S5 | 关键词：重新定位 + 差异化 |

**重点关注**：
- 5 段文案中至少 4 段推荐结果与预期一致（LLM 推断有少量随机性）
- 置信度合理（明确文案 → high；模糊文案 → medium 或 low）
- rationale 不是空话，**必须引用文案中的关键词**（如"已有产品"/"调研"等）

---

## 测试 1：S1 功能迭代

**适合的产品**：已有可对标的成熟产品 + 想看竞品功能矩阵

### 测试输入

- **分析场景**：S1 功能迭代（已有产品对标）
- **我方产品名称**：`Notion`
- **我方产品简介（选填）**：`AI 驱动的协作文档与知识管理平台，集成数据库、看板、文档于一体`
- **竞品名称（每行一个）**：
  ```
  飞书文档
  语雀
...
- **分析意图描述**：
  ```
  我们想了解飞书文档、语雀、WPS 在协作文档领域的功能矩阵，重点是 AI 写作辅助、
  多人实时协作、模板生态、移动端体验四个维度，找到我们 Notion 还没补齐的功能差距，
  以便下个版本规划迭代方向。
  ```

### 重点关注

1. 🎯 **路由验证**：后端日志应显示 `[graph] → collector` **直接进入**（不经过 recommender）
2. 🎯 **vendor_profiles 表**：
   - 至少有 3 行（飞书 / 语雀 / WPS 各一行）
   - 每行有 `wave_position`（wave_leader/wave_strong_performer/wave_contender 之一）
   - "优势数 / 警示数" 都 ≥ 1
3. 🎯 **功能矩阵**：
   - 至少 4-8 行功能（AI 写作 / 实时协作 / 模板 / 移动端等）
   - 每个竞品列都有 0/1/2 评分
...
- 雷达图缺失 → F3 plotly 没生效或竞品名不一致触发了 schema validator
- vendor_profiles 数 < 竞品数 → inspector 应报 major issue（在元数据 warnings 看）
...
- **分析意图描述**：
  ```
  我们准备进入 AI 代码生成工具市场，目前还没有产品。想调研当前赛道头部玩家、
  TAM/SAM/SOM 市场规模、Porter 五力格局，找到适合切入的细分场景与差异化策略。
  重点关注独立开发者与中小团队这两个用户群体。
  ```

### 重点关注

1. 🎯 **路由验证**：后端日志应显示 `[graph] → recommender` **先经过 recommender**，再 → collector
2. 🎯 **Recommender 推荐玩家表**：
   - 至少 3 个推荐（GitHub Copilot / Cursor / Codeium / Tabnine 之类）
...
- recommender 表为空 → Tavily API key 没配 / 搜索失败
- TAM/SAM/SOM 全是 "—" → inspector 应报 critical issue（market_sizing 全 unknown）
...
- **分析场景**：S3 定价策略
- **我方产品名称**：`Linear`
- **我方产品简介**：`面向开发团队的项目管理与 issue 跟踪工具，强调速度与键盘操作`
- **竞品名称**：
  ```
  Jira
  Asana
...
- **分析意图描述**：
  ```
  我们 Linear 当前是 per-seat 定价（$8/月），用户反馈中型团队成本过高。想看 Jira、
  Asana、ClickUp 的定价模型、套餐切分、年付折扣策略，参考 GBB 三层套餐法重新设计
  定价方案，目标是中端市场 ARR 提升 15-20%。
  ```
...
6. 🎯 **定价方案总结**：
   ...
   - `basis` 不是 `llm_inferred`（如果是 → inspector 应报 minor）
7. 🎯 **Rollout 步骤表**：≥3 步

### 失败信号

- 套餐 tier <3 → inspector 报 minor（GBB 不达三层）
- 出现 2 个 ⭐ 推荐 tier → schema validator 应直接拦截 writer 输出
...
- **分析意图描述**：
  ```
  我们要为 Mixpanel、Amplitude、PostHog 建立首次监控基线，覆盖功能变更、定价变更、
  营销文案、新闻事件、组织变化五个维度。这次是首次监控所以所有变更都是基线快照，
  下季度再跑增量对比。
  ```
...
- 首次模式下 trends 区块出现数据 → schema validator 应已拦截
- 改 prior_trace_id 为 `../../etc/passwd` → 后端日志应出现 `[graph] prior_trace_id 含非法字符，拒绝加载`，graph 仍能跑（降级首次模式）
...
- **分析场景**：S5 战略定位
- **我方产品名称**：`Figma`
- **我方产品简介**：`Web-based 协作设计工具，原生云端 + 多人实时编辑`
- **竞品名称**：
  ```
  Sketch
  Adobe XD
...
- **分析意图描述**：
  ```
  Figma 当前在专业设计师群体已经很强，但开发者与产品经理使用偏少。想做一次
  Gartner Magic Quadrant 风格的定位评估，用 ERRC 战略画布找出价值曲线差异，
  最终用 Geoffrey Moore 6 位模板写出新定位陈述，目标是把品牌从"设计师工具"
  扩展到"产品团队全员协作平台"。
  ```
...
3. 🎯 **感知地图 Perceptual Map（Plotly）**：
   - 二维散点图
   - **我方品牌（Figma）必须用 ⭐ 红色标记**（is_self=True）
   - 竞品用蓝色圆点
...
- positioning_statement.confidence=low_confidence → inspector 报 minor

---

## 验收完成判定

5 场景全跑完后，**至少满足以下 6 条**算通过：
...
- LLM 调用耗时较长，单场景从提交到出报告可能 **2-5 分钟**
- 网络抖动 / Doubao API 偶发限流 → 可能触发 graph 重试或 quality_score 偏低
- Tavily API 配额耗尽 → S2 recommender 会降级为纯 LLM 推断（confidence 全部 low）
- 首次跑 S4 增量模式（带 prior_trace_id）需要先跑过一次 S4 首次模式建立 prior

## 跑出问题怎么办

1. **前端报 ModuleNotFoundError: 'src'** → 已在最新版本修复（`from render import render_base_report`）
2. **后端报 LLM 401** → 检查 `.env` 里 `DOUBAO_API_KEY`
3. **S2 recommender 推荐为空** → 检查 `.env` 里 `TAVILY_API_KEY`
4. **图表显示"plotly 未安装，跳过图表"** → `pip install plotly>=5.20.0`
5. **整个流程无限重试** → 检查后端日志看是哪个 agent 在 raise，max_retries=2 应该会强制结束
